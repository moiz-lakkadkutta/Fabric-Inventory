...
import { usePurchaseInvoices } from '@/lib/queries/purchase-invoices';
    ...
    <div className="space-y-4">
      <header className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
        ...
        <span style={{ fontSize: 13, color: 'var(--text-tertiary)' }}>
          {pis.isPending
            ? '—'
            : `${pis.data?.length ?? 0} purchase invoice${(pis.data?.length ?? 0) === 1 ? '' : 's'}`}
        </span>
      ...
      >
        {pis.isPending ? (
          ...
          <table className="w-full text-left" style={{ minWidth: 720 }}>
            ...
            <tbody>
              {(pis.data ?? []).map((pi) => {
                const pill = STATUS_PILL[pi.status] ?? {
                ...
                return (
                  ...
                  >
                    <td className="px-3 py-3">
                      <Link
                        to={`/purchase/invoices/${pi.purchase_invoice_id}`}
                        className="mono"
                    ...
                    <td className="px-3 py-3" style={{ fontSize: 12.5 }}>
                      {pi.grn_id ? (
                        <Link to={`/purchase/grns/${pi.grn_id}`} style={{ color: 'var(--accent)' }}>
                          source GRN
                        </Link>
                      ) : (
...
function ListSkeleton({ rows }: { rows: number }) {
  return (
    <div role="status" aria-label="Loading purchase invoices" className="flex flex-col gap-2 p-4">
