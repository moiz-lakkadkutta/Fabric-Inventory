...
def _signup_body(name_suffix: str) -> dict[str, str]:
    return {
        "email": f"int10-{uuid.uuid4().hex[:6]}@example.com",
        "password": _PASSWORD,
        "org_name": f"INT10 Org {name_suffix}",
        "firm_name": f"INT10 Firm {name_suffix}",
        "state_code": _STATE,
    }
...
def test_login_response_includes_org_firm_and_available_firms(http_client) -> None:
    ...
    assert "available_firms" in data, (
        f"available_firms missing from LoginResponse: {sorted(data)}"
    )
...
def test_signup_same_email_different_org_succeeds(http_client) -> None:
    ...
    second = a.copy()
    second["org_name"] = a["org_name"] + " (other)"
